...
public class QualificationController {
  ...
  private void initialize() {
    ...
    colHorodatage.setCellValueFactory(
        cellData -> new SimpleStringProperty(cellData.getValue().getHorodatage().format(HEURE)));
    colFrequence.setCellValueFactory(
        cellData ->
            new SimpleStringProperty(
                String.format(
                    Locale.US, "%.1f kHz", cellData.getValue().getFrequenceDominanteKHz())));
    colDuree.setCellValueFactory(
        cellData -> new SimpleStringProperty(cellData.getValue().getDureeSecondes() + " s"));
